...
export class AuthService {
  ...
  static async createUserProfile(
    ...
    try {
      ...
      // Store profile data locally after successful creation/update
      if (profile) {
        await this.storeUserProfileLocally(userId, profile);
      }
  ...
  static async updateUserProfile(
    ...
    try {
      ...
      // Store updated profile data locally
      if (data) {
        await this.storeUserProfileLocally(userId, data as UserProfile);
      }
   ...
   */
  private static async storeUserProfileLocally(userId: string, profileData: Partial<UserProfile>) {
    try {
      const localProfile = {
        id: userId,
        ...profileData,
      ...
      console.log('📱 AuthService: About to store profile locally:', localProfile);
      await AsyncStorage.setItem(USER_PROFILE_KEY, JSON.stringify(localProfile));
      console.log('📱 AuthService: User profile stored locally successfully');
      
      // Verify it was stored by reading it back
      const stored = await AsyncStorage.getItem(USER_PROFILE_KEY);
      console.log('📱 AuthService: Verification - stored data:', stored ? JSON.parse(stored) : null);
    } catch (error) {
      console.error('❌ AuthService: Error storing profile locally:', error);
    }
  }
   ...
   */
  static async getUserProfileLocally(): Promise<UserProfile | null> {
    try {
      const storedProfile = await AsyncStorage.getItem(USER_PROFILE_KEY);
      console.log('📱 AuthService: Getting local profile, raw data:', storedProfile);
      if (storedProfile) {
        const parsed = JSON.parse(storedProfile);
        console.log('📱 AuthService: Parsed local profile:', parsed);
        return parsed;
      }
      console.log('📱 AuthService: No local profile found');
      return null;
    } catch (error) {
      console.error('❌ AuthService: Error getting local profile:', error);
      return null;
    }
  }

  /**
   * Update user profile both locally and in Supabase
   */
  static async updateUserProfileWithLocalSync(
    userId: string,
    profileData: Partial<UserProfile>
  ): Promise<{ profile: UserProfile | null; error: string | null }> {
    try {
      // First update locally for immediate response
      await this.storeUserProfileLocally(userId, profileData);
      
      // Then sync to Supabase in background
      const supabaseResult = await this.updateUserProfile(userId, profileData);
      
      if (supabaseResult.error) {
        console.warn('⚠️ AuthService: Supabase sync failed, but local update succeeded:', supabaseResult.error);
        // Return success since local update worked
        const localProfile = await this.getUserProfileLocally();
        return { profile: localProfile, error: null };
      }
      
      return supabaseResult;
    } catch (error) {
      console.error('❌ AuthService: Error in updateUserProfileWithLocalSync:', error);
